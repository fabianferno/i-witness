...
async function main() {
  ...
  const data = new TextEncoder().encode(
    `ðŸš€ Welcome to decentralized storage on Filecoin Onchain Cloud! 
...
main().then(() => {
  console.log("âœ… Storage workflow completed successfully!");
}).catch((error) => {
  console.error("âŒ Error occurred:");
...
:::note[Pricing]
To size your deposit, check the upâ€‘toâ€‘date rates in [**Pricing**](../introduction/about/#pricing) and use the [storage costs calculator](/developer-guides/storage/storage-costs/#detailed-calculator-guide) for a precise estimate.
:::
